...
import makeAutoUpdate from "../controllers/AutoUpdate";
  ...
  await archInterface
    ...
    .then((res) => {
      ...
      if (res.data.length === 0) {
        ui.toggleSearchWarning(`No data for ${name} was received from server in the interval ${start} to ${end}.`);
      } else {
...
async function loadFromURL() {
  // @todo: This should be a controller that will call PlotPVs use-case
  const { pvs, from, to } = Browser.getConfigFromUrl();

  if (from) {
    setStart(new Date(from));
  }
  ...
  if (to && from) {
    const newWindowTime = getNewTimeWindow();
    updateTimeWindowOnly(newWindowTime);
  } else {
    await updateStartAndEnd(new Date());
  }
